...
def get_features(csv_path,is_train=False,scaler=None):
    ...
    '''
    cols = list(pd.read_csv(csv_path, skipinitialspace=True, nrows=1))
    return pd.read_csv(
        csv_path,
        skipinitialspace=True,
        usecols=[i for i in cols if i!="shares"]
    ).to_numpy()
    # raise NotImplementedError
...
def analytical_solution(feature_matrix, targets, C=0.0):
    ...
    i = np.identity(np.size(weights, 1)) 
...
def compute_gradients(feature_matrix, weights, targets, C=0.0):
    ...
    x = np.subtract(get_predictions(feature_matrix, weights), targets)
    grad = np.dot(grad, x)
...
def sample_random_batch(feature_matrix, targets, batch_size):
    ...
    batch_size: int
    '''    
    x = np.random.randint(0, np.size(targets, 0) - batch_size)
...
def do_gradient_descent(train_feature_matrix,  
    ...
    ** However **, you ought to make use of compute_gradients and update_weights function defined above
    ...
    '''
    weights = initialize_weights(np.size(train_feature_matrix, 1))
